...
def user_home(request):
    ...
    user = request.user
    student = StudentUser.objects.filter(user=user.id)
    error = ""
    if request.method == 'POST':
...
def recruiter_home(request):
    ...
    user = request.user
    recruiter = Recruiter.objects.filter(user_id=user.id)    
    error = ""
    if request.method =='POST':
        f = request.POST['fname'] 
        l = request.POST['lname'] 
...
def add_job(request):
    ...
    if request.method == 'POST':
        ...
        user = request.user
        recruiter = Recruiter.objects.filter(user=user.id)
        try:
           Job.objects.create(recruiter_id=recruiter.id,start_date=sd,end_date=ed,title=jt,salary=sal,image=l,description=des,experience=exp,location=loc,skills=skills,creationdate=date.today())
        except:
            error = "yes" 
    d= {'error': error}    
